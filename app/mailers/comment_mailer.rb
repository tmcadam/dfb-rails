class CommentMailer < ApplicationMailer
    ...
    def comment_email(comment)
        ...
        @comment = comment
        mail(to: @comment.email, subject: 'New comment received', from: "Falklands Biographies <webeditor@falklandsbiographies.org>" )
    end

    def admin_comment_email(comment)
        # Preview @ http://localhost:3000/rails/mailers/comment_mailer/admin_comment_email
        @comment = comment
        mail(to: Rails.configuration.admin_comment_email, subject: 'New comment received')
    end
